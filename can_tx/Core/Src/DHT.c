...
void delay_us(uint32_t microseconds) {
    // Configure Timer 1
    TIM1->PSC = 0; // Prescaler to 0
    TIM1->ARR = (24 * microseconds) - 1; // Set Auto-Reload Register for the delay
    TIM1->CR1 = TIM_CR1_CEN; // Enable Timer

    while (!(TIM1->SR & TIM_SR_UIF)); // Wait until the update interrupt flag is set
